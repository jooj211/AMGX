...
void readCSRMatrix(AMGX_matrix_handle matrix, AMGX_vector_handle rhs, AMGX_vector_handle soln, std::string problem)
{
    ...
    int *aux_row_ptr = new int[n/3 + 1];
    int *aux_col_ind = new int[n/3];
    double *aux_values = new double[nnz/9];

    for (int i = 0; i < n/3 + 1; i++){
        aux_row_ptr[i] = row_ptr[i];
    }

    for (int i = 0; i < n/3; i++){
        aux_col_ind[i] = col_ind[i];
    }

    for (int i = 0; i < nnz/9; i++){
        aux_values[i] = values[i];
    }



    AMGX_matrix_upload_all(matrix, n/3, nnz/9, 3, 3, row_ptrs, col_indices, values, NULL);
    ...
    AMGX_vector_upload(rhs, n/3, 3, rhs_values);
    AMGX_vector_upload(soln, n/3, 3, soln_values);
}
...
int main(int argc, const char **argv)
{
    ...
    */
    ...
    else if (std::string(argv[3]) == "-p")
    {
        ...
        if (problem == "cube-poisson" || problem == "ve-mech" || problem == "ve-poisson")
        {
        ...
        {
            cout << "Invalid problem: " << problem << endl;
            cout << "Valid entries are: cube-poisson, ve-mech, ve-poisson" << endl;
        }
    }
